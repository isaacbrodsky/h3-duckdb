...
tidy-check:
	mkdir -p ./build/tidy
	cmake -DEXTENSION_NAME=${EXTENSION_NAME} -DDISABLE_UNITY=1 -DCLANG_TIDY=1 -S . -B build/tidy
	cp duckdb/.clang-tidy build/tidy/.clang-tidy
	cd build/tidy && python3 ../../duckdb/scripts/run-clang-tidy.py 'src/.*/' -header-filter 'src/.*/' -quiet ${TIDY_THREAD_PARAMETER} ${TIDY_BINARY_PARAMETER} ${TIDY_PERFORM_CHECKS}
